...
The capture hub uses large guest-facing mode cards instead of administrative controls. Three-shot mode keeps “Photo n of 3” visible, announces countdown starts without reading every number, gives every review image an ordered text alternative, and moves focus to review and completion headings. Layout previews reserve their template aspect ratio to prevent layout shift. Automatic timing and camera reliability never depend on animation.

Video Booth follows the same immersive portrait-first surface and safe-area spacing. Recording status combines text and color in one atomic live status, controls remain 44px or taller, camera switching is disabled while recording, and navigation is unavailable during recording/processing. The eight-second limit is timer-driven rather than animation-driven. Captured, accepted, and error headings receive focus; preview uses native controls and `playsInline`. Retake, Use Video, Download Video, and Back to Capture Modes remain explicit native actions.

Saving uses explicit Preparing, Uploading, Finishing, Saved, and retry feedback while keeping device download available. The guest gallery remains dark and mobile-first: two fixed-aspect columns on phones, three or four at wider breakpoints, lazy optimized images, 44px controls, a helpful empty state, and native share with clipboard fallback. Host moderation uses the existing light workspace tokens and never exposes raw object paths.

Finalize retry keeps the same media identity after upload and communicates recovery beside the save control through an announced message. A transient network/server failure retries finishing rather than silently creating a duplicate gallery item.
